    ...
    def set_mode(self, mode):
        ...
        if mode in ['peak_pick', 'peak_delete']:
            if self.raw_data is None or self.raw_data.ndim != 2:
                QMessageBox.information(self, "Feature in Progress", f"Peak picking is currently only supported for 2D spectra.")
                mode = None 
            ...
            if self.current_mode in ['peak_pick', 'peak_delete']:
                ...
                if self.current_mode == 'peak_pick':
                    if event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
                        # Force pick bypasses the noise check entirely
                        self.peak_manager.add_force_peak(click_ppm_x, click_ppm_y)
...
#---------------------------------------------------------------------        
    ...
    def create_menus(self):
        ...
        peaks_menu.addAction(clear_peaks_action)
            ...
            try:
                ...
                with open(file_path, "w") as f:
                    if ndim == 1:
                        ...
                        # Loop through the manager's list!
...
#---------------------------------------------------------------------        
    ...
    def force_pick(self):
        if self.raw_data is not None:
            ...
            if self.raw_data.ndim != 2:
                QMessageBox.information(self, "Feature in Progress", "Force Picking is currently only supported for 2D spectra.")
                return
...
#---------------------------------------------------------------------        
    ...
    def update_peak_markers(self):
        ...
        # Get the clean data from our manager
        ...
        # Clean up deleted peaks from the UI
...
#---------------------------------------------------------------------        
    ...
    def auto_pick(self):
        ...
        if vis_data.ndim != 2:
            QMessageBox.information(self, "Feature in Progress", "Auto Peak Picking is currently only supported for 2D spectra.")
            return    
                    
        # Calculate the exact noise threshold currently displayed on screen
        base_mult = self.cont_sliders['base'].value()
        noise_rmsd = self.data_handler.calculate_rmsd(vis_data)
        threshold = noise_rmsd * base_mult
        
        # Prevent accidentally piling thousands of peaks on top of each other
